...
import dotterweide.metals.MetalsActor._
...
class ScalaParser(scalaVersion: Version, prelude: String, postlude: String) extends Parser with Adviser {
  ...
  private[this] val compilerActor: ActorRef = system.actorOf(Props(
    new MetalsActor(scalaVersion = scalaVersion, prelude = prelude, postlude = postlude)
  ), "compiler")
  ...
  def parseAsync(text: String, tokens: Iterator[Token])(implicit async: Async): Future[Node] = {
    ...
    val fut = compilerActor.ask(Compile(text))(Timeout(60 /*5*/, TimeUnit.SECONDS)).mapTo[Node]
    fut.onComplete {
      case Success(_) =>
      case Failure(ex) =>
  ...
  def typeAtAsync(document: Document, data: Data, offset: Int)
    ...
    // XXX TODO --- timeout is arbitrary
    val fut = compilerActor.ask(Type(document.text, offset))(Timeout(60 /*5*/, TimeUnit.SECONDS)).mapTo[Option[NodeType]]
    fut.onComplete {
      case Success(_) =>
      case Failure(ex) =>
        println("Type failed:")
  ...
  def variantsAsync(document: Document, data: Data, offset: Int)
    ...
    // XXX TODO --- timeout is arbitrary
    val fut = compilerActor.ask(Complete(document.text, offset))(Timeout(60 /*5*/, TimeUnit.SECONDS)).mapTo[Adviser.Result]
    fut.onComplete {
      case Success(_) =>
      case Failure(ex) =>
